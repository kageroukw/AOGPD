...
namespace AOGPD
{
    public class AuthorizePageHandlerFilter : IAsyncPageFilter, IOrderedFilter
    {
        ...
        private readonly IPolicyEvaluator policyEval;
        ...
        private async Task AuthorizeAsync(ActionContext actionContext, AuthorizationPolicy policy)
        {
            ...
            var authenticateResult = await policyEval.AuthenticateAsync(policy, httpContext);
            var authorizeResult = await policyEval.AuthorizeAsync(policy, authenticateResult, httpContext, actionContext.ActionDescriptor);
